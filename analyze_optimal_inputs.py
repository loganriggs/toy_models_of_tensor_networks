...
LAYER 0 ANALYTICAL:
  ...
  visualize_optimal_input(opt_input)   - Show optimal input as 8x8 heatmap per embed dim
...
# %%
# EXAMPLE 1: Analyze a single Layer 0 channel
analyze_channel_layer0(channel=0)

# %%
# EXAMPLE 2: Analyze a single Layer 1 channel
analyze_channel_layer1(channel=0, n_steps=300)

# %%
# EXAMPLE 3: Compute all Layer 0 optimal inputs and statistics
pos_optimal, neg_optimal, pos_eig, neg_eig, alignments = compute_all_layer0_stats()

# %%
# EXAMPLE 4: Find channels with highest/lowest L-R alignment
...
# %%
# EXAMPLE 5: Compare optimal vs real activations for a channel
channel = 0
result = get_layer0_optimal_input(channel)
print(f"\nChannel {channel}:")
...
# %%
# EXAMPLE 6: Show top activating for multiple channels
for ch in [0, 10, 50, 100]:
    print(f"\n--- Channel {ch} ---")
    show_top_activating(ch, layer=0, k=3)

# %%
# EXAMPLE 7: Layer 1 optimization for multiple channels
for ch in [0, 50]:
    print(f"\n--- Layer 1, Channel {ch} ---")
